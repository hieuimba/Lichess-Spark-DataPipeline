...
def filter_games(df):
    ...
    df = df.withColumn(
        "EloRange",
        (floor(((col("WhiteElo") + col("BlackElo")) / 2) / 300) * 300).cast("int"),
    )

    # Adjust "EloRange" for special cases
    df = df.withColumn(
        "EloRange",
        when(col("EloRange") <= 300, 0)
    ...
    df = df.withColumn(
        "Moves",
        concat(
            lit("{ "), col("ECO"), lit(" "), col("Opening"), lit(" } "), col("Moves")
        ),
    )
    ...
    df = df.withColumn(
        ...
        .when(
            (col("Result") == "1-0") & (col("Termination") == "Time forfeit"),
            "White wins on time",
        )
        .when(
            (col("Result") == "0-1") & (col("Termination") == "Time forfeit"),
            "Black wins on time",
        )
        .when(
            (col("Result") == "1-0")
            & (col("Termination") == "Normal")
            & when(regexp_extract(col("Moves"), r"#\s", 0) != "", True),
    ...
    df = df.withColumn(
        "Moves", concat(col("Moves"), lit(" { "), col("ResultComment"), lit(" }"))
    )
    ...
    df = df.withColumn(
        "Event",
        trim(
            regexp_replace(df["Event"], "(http\S+)|(game|tournament|Rated|swiss)", "")
        ),
    )
    # Replace "Standard" with "Rapid" in "Event" column
    df = df.withColumn(
        "Event", when(col("Event") == "Standard", "Rapid").otherwise(col("Event"))
    )
