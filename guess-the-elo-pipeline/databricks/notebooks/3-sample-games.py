...
all_df = all_df.withColumn(
    "GameID", concat(lit(file_month), lit("-"), col("NumericGameID"))
)
...
key_df = all_df.groupBy("Event").agg(
    min("NumericGameID").alias("MinGameID"), max("NumericGameID").alias("MaxGameID")
)
key_df = key_df.withColumn("Month", lit(file_month))
key_df = key_df.withColumn(
    "KeyID", concat(lit(file_month), lit("-"), lit(col("Event")))
)
...
def read_and_combine_dataframes(keys_path):
    ...
    folders = [
        file.path
        for file in dbutils.fs.ls(keys_path)
        if "Consolidated" not in file.path
    ]
    print(folders)
...
df = df.withColumn("Range", struct("Min", "Max")).drop("Min", "Max")
